  ...
  if(setting)
    ...
    if(setting->type == CONFIG_TYPE_STRING)
      _delete(setting->value.sval);
            ...
            || (setting->type == CONFIG_TYPE_RANGE)
...
static int __config_array_checktype(const config_setting_t *array, int type)
  ...
  elem = array->value.list->elements[0];
...
void __config_write(const config_setting_t *setting, FILE *stream, int depth)
  {
  config_list_t *list;
...
int config_setting_range_set_value(config_setting_t *range, int index, long value)
  {
  config_list_t *list = range->value.list;
  config_setting_t *element = NULL;

  if(range->type != CONFIG_TYPE_RANGE)
    return(CONFIG_FALSE);
  ...
  if(index < 0)
    ...
    element = config_setting_create(range, NULL, CONFIG_TYPE_INT);
    }
  else
    {
    element = config_setting_get_elem(range, index);
...
int config_setting_range_get_min(const config_setting_t *range)
  {
  config_list_t *list = range->value.list;
  int index = 0;
  
  if(range->type != CONFIG_TYPE_RANGE || !list)
    return INT_MAX;

  if(index >= list->length)
    return INT_MAX;

  return(list->elements[index]->value.ival);
  }

int config_setting_range_get_max(const config_setting_t *range)
  {
  config_list_t *list = range->value.list;
  int index = 1;
...
config_setting_t *config_setting_add(config_setting_t *parent,
                                     const char *name, int type)
  {
  if((type < CONFIG_TYPE_NONE) || (type > CONFIG_TYPE_RANGE))
    return(NULL);
  ...
  if(name) {
    config_setting_t *tmp = config_setting_get_member(parent, name);
    if(tmp && tmp->type == type) {
      return tmp; // already exists but it's the same element
    } else if (tmp) {
      return(NULL); // already exists, and it's type differs
    }
  }
...
int config_setting_destroy(config_setting_t *setting) {
  if(!setting)
    return(CONFIG_FALSE);
...
int config_setting_delete_child(config_setting_t *parent, const char *name)
  {
  int index;
  config_setting_t *setting;
...
config_setting_t *config_setting_remove_child(config_setting_t *parent, const char *name)
  {
  int index;
  config_setting_t *setting;
...
config_setting_t *config_setting_remove_index(config_setting_t *parent, int index)
  {
  config_setting_t *setting;
  ...
  setting = config_setting_get_elem(parent, index);
