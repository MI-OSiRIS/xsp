...
int xsp_init_curl(xspCURLContext *cc, long flags) {
    ...
    // Initialize CURL with cflags
    res = curl_global_init(cflags);
    if (res != CURLE_OK) {
...
char *xsp_curl_json_string(xspCURLContext *cc, char *target, int curl_opt, char *send_str, char **ret_str) {
    ...
    /*!  @todo { we need to define some content-types this little library supports
...
static size_t write_cb(void *ptr, size_t size, size_t nmemb, void *userp) {
    ...
    // char ptr to catch return value of realloc (to supress compiler warning)
    char *returnPtr;
    ...
    if (!(data->ptr)) {
        ...
        returnPtr = realloc(data->ptr, (dsize + data->len) * sizeof (char));
    }
