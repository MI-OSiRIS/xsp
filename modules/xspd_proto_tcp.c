...
int xspd_proto_tcp_init();
static xspdConn *xspd_proto_tcp_connect(const char *hop_id, xspdSettings *settings);
static xspdListener *xspd_proto_tcp_setup_listener(const char *listener_id, xspdSettings *settings, int one_shot, listener_cb callback, void *arg);
static int xspd_proto_tcp_start_listener(xspdListener *listener);
static int xspd_proto_tcp_stop_listener(xspdListener *listener);
static void xspd_proto_tcp_free_listener(xspdListener *listener);
static void *xspd_tcp_accept_handler(void *v);

static xspdProtocolHandler xspd_tcp_handler = {
	.connect = xspd_proto_tcp_connect,
	.setup_listener = xspd_proto_tcp_setup_listener,
	.name = "tcp"
};

static xspdModule xspd_tcp_module = {
	.desc = "TCP Handler Module",
	.dependencies = "tcp", 			// XXX: gross hack
	.init = xspd_proto_tcp_init
};

static int web100_available;

xspdModule *module_info() {
	return &xspd_tcp_module;
}
...
int xspd_proto_tcp_init() {
...
#ifdef HAVE_WEB100
	if (xspd_web100_init() != 0) {
		xspd_warn(0, "warning: web100 initialization failed");
		web100_available = 0;
	} else {
		web100_available = 1;
	}
	...
	if (xspd_add_protocol_handler(&xspd_tcp_handler)) {
...
static xspdConn *xspd_proto_tcp_connect(const char *hostname, xspdSettings *settings) {
	...
	if (xspd_settings_get_int_2(settings, "tcp", "port", &port) != 0) {
		port = tcp_def_outgoing_port;
	}

	if (xspd_settings_get_int_2(settings, "tcp", "recv_bufsize", &recv_bufsize) != 0) {
		recv_bufsize = tcp_def_outgoing_recv_bufsize;
	}

	if (xspd_settings_get_int_2(settings, "tcp", "send_bufsize", &send_bufsize) != 0) {
		send_bufsize = tcp_def_outgoing_send_bufsize;
	}

	if (xspd_settings_get_int_2(settings, "tcp", "recv_timeout", &recv_timeout) != 0) {
		recv_timeout = tcp_def_incoming_recv_timeout;
	}

	if (xspd_settings_get_int_2(settings, "tcp", "send_timeout", &send_timeout) != 0) {
		send_timeout = tcp_def_incoming_send_timeout;
	}

#ifdef HAVE_WEB100
	if (xspd_settings_get_bool_2(settings, "tcp", "use_web100", &use_web100) != 0) {
		use_web100 = web100_available;
	} else if (use_web100 == 1 && web100_available == 0) {
		xspd_warn(0, "requested web100 for tcp connection to %s, but web100 support is currently unavailable", hostname);
		use_web100 = 0;
	}
#else
	use_web100 = 0;
	...
	if (xspd_settings_get_bool_2(settings, "tcp", "use_ipv6", &use_ipv6) != 0) {
		use_ipv6 = tcp_def_use_ipv6;
	}

	if (xspd_settings_get_2(settings, "tcp", "interface", &bind_interface) != 0) {
		bind_interface = NULL;
	}

	xspd_info(1, "connecting to %s/%d", hostname, port);
	...
	if (retval != 0) {
		xspd_err(1, "lookup of \"%s\" failed", hostname);
		goto error_exit;
	}
	...
	if (!connected) {
		xspd_err(0, "couldn't connect to \"%s\"", hostname);
		goto error_exit;
	}

	ret_conn = xspd_conn_tcp_alloc(new_sd, use_web100);
	if (!ret_conn) {
		xspd_err(10, "couldn't allocate socket structure");
		goto error_exit_sd;
	}

	xspd_info(1, "connected to %s", hostname);
...
static xspdListener *xspd_proto_tcp_setup_listener(const char *listener_id, xspdSettings *settings, int one_shot, listener_cb callback, void *arg) {
	...
	new_listener = xspd_listener_alloc();
	if (!new_listener)
		goto error_exit;
	...
	new_listener->settings = xspd_settings_duplicate(settings);
	...
	new_listener->start = xspd_proto_tcp_start_listener;
	new_listener->stop = xspd_proto_tcp_stop_listener;
	new_listener->free = xspd_proto_tcp_free_listener;

	return new_listener;

error_exit_listener:
	xspd_listener_free(new_listener);
error_exit:
	return NULL;
}
...
static int xspd_proto_tcp_start_listener(xspdListener *listener) {
	...
	// grab the tcp settings from the given policy
	if (xspd_settings_get_int_2(listener->settings, "tcp", "port", &port) != 0) {
		port = tcp_def_incoming_port;
	}

	if (xspd_settings_get_int_2(listener->settings, "tcp", "recv_bufsize", &recv_bufsize) != 0) {
		recv_bufsize = tcp_def_incoming_recv_bufsize;
	}

	if (xspd_settings_get_int_2(listener->settings, "tcp", "send_bufsize", &send_bufsize) != 0) {
		send_bufsize = tcp_def_incoming_send_bufsize;
	}

	if (xspd_settings_get_list_2(listener->settings, "tcp", "interfaces", &interfaces, &num_interfaces) != 0) {
		interfaces = NULL;
	}

	if (xspd_settings_get_bool_2(listener->settings, "tcp", "use_ipv6", &use_ipv6) != 0) {
		use_ipv6 = tcp_def_use_ipv6;
	}
	...
	if (interfaces != NULL) {
		...
		if (!sockets) {
			xspd_err(0, "couldn't listen on port: %d", port);
			goto error_exit;
		}
	} else {
		if (use_ipv6) {
			xspd_tcp_family = AF_INET6;
		} else {
			xspd_tcp_family = AF_INET;
		}

		sockets = listen_port(IPPROTO_TCP, xspd_tcp_family, port, &num_sockets, &ai);
		if (!sockets) {
			xspd_err(0, "couldn't listen on port: %d", port);
			goto error_exit;
		}
	...
	xspd_info(0, "TCP: num sockets: %d %d", num_sockets, sockets[0]);
	...
	if (pthread_create(&tcp_args->accept_thread, NULL, xspd_tcp_accept_handler, (void *) listener) != 0) {
		xspd_err(0, "couldn't execute socket handler thread: %s", strerror(errno));
		goto error_exit_sockets;
	}

	listener->status = LISTENER_RUNNING;

	xspd_info(0, "Started TCP listener: %s", listener->id);
...
static int xspd_proto_tcp_stop_listener(xspdListener *listener) {
	...
	if (listener->status != LISTENER_RUNNING) {
		xspd_err(0, "tried to stop a stopped listener: %d", listener->status);
		return -1;
	}

	if ((n = pthread_cancel(tcp_args->accept_thread)) < 0){
		xspd_err(0, "pthread_cancel failed: %d, %s", n, strerror(errno));
		return -1;
	}

	if ((n = pthread_join(tcp_args->accept_thread, NULL)) < 0){
		xspd_err(0, "pthread_join failed: %d, %s", n, strerror(errno));
		return -1;
	}
...
static void xspd_proto_tcp_free_listener(xspdListener *listener) {
	...
	if (listener->status != LISTENER_STOPPED)
		xspd_proto_tcp_stop_listener(listener);
...
static void *xspd_tcp_accept_handler(void *v) {
...
#ifdef HAVE_WEB100
	if (xspd_settings_get_bool_2(listener->settings, "tcp", "use_web100", &use_web100) != 0) {
		use_web100 = web100_available;
	} else if (use_web100 == 1 && web100_available == 0) {
		xspd_warn(0, "requested web100 for tcp listener but web100 support is currently unavailable");
		use_web100 = 0;
	}
#else
	use_web100 = 0;
	...
	if (xspd_settings_get_int_2(listener->settings, "tcp", "send_timeout", &send_timeout) != 0) {
		send_timeout = tcp_def_incoming_send_timeout;
	}
	
	if (xspd_settings_get_int_2(listener->settings, "tcp", "recv_timeout", &recv_timeout) != 0) {
		recv_timeout = tcp_def_incoming_recv_timeout;
	}
	...
	while(1) {
		...
		//xspd_info(1, "Coming out of select");
		...
		for(i = 0; i < tcp_args->num_sockets; i++) {
			...
			//xspd_info(0, "new incoming connection: %s", new_conn->description);
			...
			if (recv_timeout > 0) {
				...
				if ((setsockopt(sd, SOL_SOCKET, SO_RCVTIMEO, &new_to, sizeof(struct timeval))) < 0) {
					xspd_warn(1, "failed to set recv timeout of \"%s\" to \"%d\"",
						 new_conn->description,
						 recv_timeout);
				}
			}
			
			//xspd_info(0, "post recv timeout: %s", new_conn->description);
			...
			if (send_timeout > 0) {
				...
				if ((setsockopt(sd, SOL_SOCKET, SO_SNDTIMEO, &new_to, sizeof(struct timeval))) < 0) {
					xspd_warn(1, "failed to set send timeout of \"%s\" to \"%d\"",
						 new_conn->description,
						 recv_timeout);
				}
			}
			
			//xspd_info(0, "post send timeout: %s", new_conn->description);
			
			new_conn->settings = xspd_settings_duplicate(listener->settings);
			
			//xspd_info(0, "post settings duplicate: %s", new_conn->description);
			
			n = listener->callback(listener, new_conn, listener->arg);
			if (n == 0 && listener->one_shot)
				goto out;
			
			//xspd_info(0, "post call-back: %s", new_conn->description);
		}
 ...
 out:
	xspd_info(1, "Closing all sockets and exiting listener");
