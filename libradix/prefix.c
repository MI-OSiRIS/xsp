...
// =============================================================================
/* 
 * $Id: prefix.c,v 1.4 2004/10/13 04:35:01 swany Exp $
 * $Imported: prefix.c,v 1.1 2003/06/11 03:14:45 swany Exp $
 * from MRT
 */
...
New_Prefix2 (int family, void *dest, int bitlen, prefix_t *prefix)
{
    ...
    if (family == AF_INET6) {
        default_bitlen = 128;
	if (prefix == NULL) {
            prefix = (prefix_t *) malloc(sizeof(prefix6_t));
	    dynamic_allocated++;
	}
    ...
    if (family == AF_INET) {
		if (prefix == NULL) {
#ifndef NT
            prefix = (prefix_t *) malloc(sizeof(prefix4_t));
#else
...
prefix_toname (prefix_t *prefix)
{
    ...
    name = (char *) malloc(sizeof(char) * (strlen(hostinfo->h_name) + 1));
...
prefix_toa2x (prefix_t *prefix, char *buff, int with_len)
{
	...
	buffp = (struct buffer *) malloc(sizeof(struct buffer));
...
prefix_tosockaddr(prefix_t *prefix)
{
	...
	sockaddr6 = malloc(sizeof(struct sockaddr_in6));
	if (!sockaddr6)
	    return NULL;
...
int is_ipv4_prefix (char *string) {
  ...
  while (cp != NULL) {
    octet++;
    if ((atoi (cp) < 0) || (atoi (cp) > 255)) {free(copy); return (-1);}
    cp = strtok_r (NULL, ".", &last);
  }

  if ((octet > 4) || (octet <= 0)) {free(copy); return (-1);}
