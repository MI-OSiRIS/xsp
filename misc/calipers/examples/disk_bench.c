...
static const volatile char rcsid[] = "$Id: disk_bench.c 33086 2012-11-12 18:23:45Z dang $";

#define BLOCK1MB 1048576
#define BLOCK 131072
#define RPT_INTERVAL 10000
...
static int run(device_t src, device_t dst, int size, output_t outp) {
  ...
  for (i=0; i < 2; i++) {
    ...
    if (!getenv("NETLOGGER_HISTOGRAM_OFF")) {
      printf("Histogram ON\n");
