...
struct xsp_tpool_config_t {
	unsigned int min_threads;
};

static struct xsp_tpool_config_t xspTpoolConfig = {
	.min_threads = 1
};

void xsp_tpool_read_config() {
	int val;

	if (xsp_main_settings_get_int("tpool", "min_threads", &val) == 0) {
		if (val >= 0)
			xspTpoolConfig.min_threads = val;
	}
}
...
int xsp_tpool_init() {
	...
	xsp_tpool_read_config();

	// initialize the add mutex
	if (pthread_mutex_init(&tpool_add_lock, 0) != 0) {
		fprintf(stderr, "xsp_tpool_init(): couldn't initialize thread pool lock\n");
		goto error_exit;
	}

	if (pthread_attr_init((pthread_attr_t *)&pt_attr) != 0) {
		fprintf(stderr, "xsp_tpool_init(): couldn't initialize thread attributes\n");
		goto error_exit2;
	}

#if !defined(FREEBSD)
	if (pthread_attr_setscope((pthread_attr_t *)&pt_attr, PTHREAD_SCOPE_SYSTEM) != 0) {
		fprintf(stderr, "xsp_tpool_init(): couldn't set thread scope\n");
		goto error_exit2;
	}
#endif
	...
	pool = (struct tpool_entry **) malloc(sizeof(struct tpool_entry *) * xspTpoolConfig.min_threads);
	if (!pool) {
		fprintf(stderr, "xsp_tpool_init(): error allocating thread pool\n");
		goto error_exit3;
	}

	// NULL the pointer list
	bzero(pool, sizeof(struct tpool_entry *) * xspTpoolConfig.min_threads);

	// allocate new thread pool entries
	for(i = 0; i < xspTpoolConfig.min_threads; i++)
	{
		pool[i] = alloc_tpool_entry();
		if (pool[i] == NULL) {
			fprintf(stderr, "xsp_tpool_init(): error allocating initial threads\n");
			goto error_exit4;
		}
	}

	pool_size = xspTpoolConfig.min_threads;
...
/*
 *  void xsp_tpool_exec(void *(*fn) (void *), void *arg):
